...
public:
  set() : tree_(new tree_type()), size_ (0) {}
  set(std::initializer_list<value_type> const &items);
  set(const set &s);
  set(set &&s);
  ~set() {
    tree_.DeleteTreeRecursive();
    size_ = 0;
  }
  operator=(set &&s);
  ...
  std::pair<iterator, bool> insert(const value_type& value){
    // std::pair<iterator,bool> pair = tree_->InsertUniq(std::make_pair(key,0));
    return tree_->InsertUniq(std::make_pair(value, 0));
  }
