#ifndef __S21_MYSTACK_H__
#define __S21_MYSTACK_H__
...
class myStack {
  ...
  myStack(std::initializer_list<value_type> const &items) : pnext_(nullptr) , size_(0) {
  ...
  template <class... Args>
  void emplace_back(Args&&... args){ // вызывает конструктор элемента и создает обьект внутри себя
  ...
  bool operator==(const myStack& other) noexcept { return size_ == other.size_; };
  bool operator!=(const myStack& other) noexcept { return size_ != other.size_; };
  ...
  struct StackNode_
  {
    ...
    explicit StackNode_(value_type data = value_type(), StackNode_* pnext = nullptr) 
